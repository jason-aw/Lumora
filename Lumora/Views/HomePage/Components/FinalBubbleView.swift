...
struct FinalBubbleView: View {
    ...
    var body: some View {
        ...
        .blendMode(.screen)
        .blur(radius: blur)
        .scaleEffect(1 + breatheAmount)
        .rotationEffect(.degrees(wobble ? 1.2 : -1.2))
        // Keep existing repeating motions
        .animation(.easeInOut(duration: 9.0 / animationSpeed).repeatForever(autoreverses: true), value: wobble)
        // Explicitly animate volume-driven scale changes
        .animation(.easeInOut(duration: 0.12), value: breatheAmount)
        .onAppear {
            breathe = true
            wobble = true
...
#Preview {
    ZStack {
        ...
        FinalBubbleView(size: 234, blur: 12 , animationSpeed: 10.0, volume: .constant(1.0))
    }
}
